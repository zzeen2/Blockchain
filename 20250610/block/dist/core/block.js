...
class Block {
    ...
    static craeteBlockHash(_block) {
        const { version, previousHash, timestamp, merkleRoot, height, nonce, deffculty } = _block;
        // 
        const value = `${version}${previousHash}${timestamp}${height}${merkleRoot}${nonce}${deffculty}`;
    ...
    static findBlock(generateBlock) {
        ...
        while (true) {
            ...
            const result = binary.startsWith("0".repeat(generateBlock.deffculty));
            console.log(binary);
            console.log(generateBlock.deffculty);
            if (result) {
                // 정답을 맞췄다 
                // 블록을 추가할 권한을 얻은것.
